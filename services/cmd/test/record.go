...
func NewTracker(ctx context.Context, creds LivekitCredentials, roomID string) (*Tracker, error) {
	tracker := &Tracker{
		roomID: roomID,
	}
...
func (t *Tracker) writeTrack(ctx context.Context, track *webrtc.TrackRemote, pli lksdk.PLIWriter, kind rtc.TrackKind, source rtc.TrackSource, startAt time.Time) {
	...
	recordID := uuid.New()
	objectPath := path.Join(t.roomID, recordID.String())
	tmpFilePath := path.Join(t.tmpDir, fmt.Sprintf("%s_%s", t.roomID, recordID.String()))
